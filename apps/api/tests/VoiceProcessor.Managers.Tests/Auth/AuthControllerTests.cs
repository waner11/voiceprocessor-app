...
public class AuthControllerTests
{
    ...
    public async Task GetCurrentUser_WithValidUser_FetchesActualCreditsFromDatabase()
    {
        ...
        var userInfoResponse = new Domain.DTOs.Responses.Auth.UserInfoResponse
        {
            Id = userId,
        ...
        _mockAuthManager.Setup(x => x.GetCurrentUserAsync(userId, It.IsAny<CancellationToken>()))
        ...
        var response = okResult.Value.Should().BeOfType<Domain.DTOs.Responses.Auth.UserInfoResponse>().Subject;
        response.CreditsRemaining.Should().Be(creditsRemaining);
        response.Id.Should().Be(userId);
        response.Email.Should().Be("test@example.com");
        response.Name.Should().Be("Test User");
        response.Tier.Should().Be(SubscriptionTier.Free);

        _mockAuthManager.Verify(x => x.GetCurrentUserAsync(userId, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetCurrentUser_UserNotFound_ReturnsNull()
    {
        ...
        _mockAuthManager
            ...
            .ReturnsAsync(null as Domain.DTOs.Responses.Auth.UserInfoResponse);
    ...
    {
        // Arrange - Test with representative credit amount to ensure actual values are returned
        var userId = Guid.NewGuid();
        var creditsRemaining = 5000;
        ...
        var okResult = result.Should().BeOfType<OkObjectResult>().Subject;
        var response = okResult.Value.Should().BeOfType<Domain.DTOs.Responses.Auth.UserInfoResponse>().Subject;
        response.CreditsRemaining.Should().Be(creditsRemaining, $"Expected {creditsRemaining} credits but got {response.CreditsRemaining}");
    }
    ...
    [Fact]
    public void GetOAuthUrl_WhenRedirectUriMissing_ReturnsBadRequestWithErrorResponse()
    {
        // Act
    ...
    [Fact]
    public void GetOAuthUrl_WhenInvalidProvider_ReturnsBadRequestWithErrorResponse()
    {
        // Arrange
